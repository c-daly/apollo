...
- When you start work on an issue that lives on the LOGOS workspace project, move its card to *In Progress* (and update any `status/*` label). Once a PR is open and just waiting for review/merge, move it to *PR awaiting merge*. When the PR merges or the task finishes, move it to *Done* so the board stays accurate for the rest of the team.
