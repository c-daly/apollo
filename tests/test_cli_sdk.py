...
def mock_sophia_client():
    ...
    client.get_state = Mock(return_value=SophiaResponse(
        success=True,
...
def mock_hermes_client():
    ...
    client.embed_text = Mock(return_value=HermesResponse(
        success=True,
...
def mock_persona_client():
    ...
    client.create_entry = Mock(return_value={
        "id": "entry-123",
        "entry_type": "thought",
        "content": "Test entry"
    })
...
class TestCLIUsesSDKClients:
    ...
    def test_cli_initializes_sdk_clients(self, cli_runner, mock_config, mock_sophia_client, mock_hermes_client, mock_persona_client):
        """Test CLI initialization creates SDK client instances."""
        with patch("apollo.cli.main.ApolloConfig.load", return_value=mock_config), \
             patch("apollo.cli.main.SophiaClient", return_value=mock_sophia_client) as sophia_constructor, \
             patch("apollo.cli.main.HermesClient", return_value=mock_hermes_client) as hermes_constructor, \
             patch("apollo.cli.main.PersonaClient", return_value=mock_persona_client) as persona_constructor:
    ...
    def test_status_command_uses_sophia_client(self, cli_runner, mock_config, mock_sophia_client, mock_hermes_client, mock_persona_client):
    ...
    def test_state_command_uses_sophia_client(self, cli_runner, mock_config, mock_sophia_client, mock_hermes_client, mock_persona_client):
    ...
    def test_cli_no_direct_requests_calls(self, cli_runner, mock_config, mock_sophia_client, mock_hermes_client, mock_persona_client):
...
class TestCLIErrorHandling:
    ...
    def test_status_handles_connection_failure(self, cli_runner, mock_config, mock_hermes_client, mock_persona_client):
    ...
    def test_state_handles_api_error(self, cli_runner, mock_config, mock_hermes_client, mock_persona_client):
        """Test state command handles API errors gracefully."""
        failing_client = Mock(spec=SophiaClient)
        failing_client.get_state = Mock(return_value=SophiaResponse(
            success=False,
            data=None,
        ...
        with patch("apollo.cli.main.ApolloConfig.load", return_value=mock_config), \
            ...
            assert result.exit_code == 0
            assert "Failed" in result.output or "Error" in result.output or "timeout" in result.output.lower()


class TestCLIConfigLoading:
    """Test CLI configuration loading."""

    def test_cli_loads_default_config(self, cli_runner, mock_sophia_client, mock_hermes_client, mock_persona_client):
        """Test CLI loads default config when no file specified."""
        with patch("apollo.cli.main.ApolloConfig.load") as mock_load, \
             patch("apollo.cli.main.SophiaClient", return_value=mock_sophia_client), \
             patch("apollo.cli.main.HermesClient", return_value=mock_hermes_client), \
             patch("apollo.cli.main.PersonaClient", return_value=mock_persona_client):
    ...
    def test_cli_loads_custom_config_file(self, cli_runner, mock_sophia_client, mock_hermes_client, mock_persona_client, tmp_path):
...
""")
        ...
        with patch("apollo.cli.main.ApolloConfig.load") as mock_load, \
            ...
            result = cli_runner.invoke(cli, ["--config", str(config_file), "status"])
...
class TestCLIOutputFormatting:
    ...
    def test_status_displays_formatted_output(self, cli_runner, mock_config, mock_sophia_client, mock_hermes_client, mock_persona_client):
    ...
    def test_state_displays_yaml_format(self, cli_runner, mock_config, mock_sophia_client, mock_hermes_client, mock_persona_client):
...
class TestCLIClientIntegration:
    ...
    def test_cli_context_contains_all_clients(self, cli_runner, mock_config, mock_sophia_client, mock_hermes_client, mock_persona_client):
    ...
    def test_multiple_commands_share_client_instances(self, cli_runner, mock_config, mock_sophia_client, mock_hermes_client, mock_persona_client):
