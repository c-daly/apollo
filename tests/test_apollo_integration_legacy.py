...
These tests require actual service dependencies to be running:
- Neo4j: For HCG graph data (Apollo connects directly)
- Hermes: For LLM chat functionality

NOTE: These integration tests are SKIPPED by default. To run them:
    RUN_INTEGRATION_TESTS=1 pytest tests/test_apollo_integration.py -m integration
...
def skip_integration_tests():
    ...
    This requires real services (Neo4j, Hermes) to be running.
    """
    if not os.getenv("RUN_INTEGRATION_TESTS"):
        pytest.skip(
            "Integration tests require real services (Neo4j, Hermes). "
...
def integration_config():
    ...
    if not config.hermes.host:
        pytest.skip("Integration tests require Hermes configuration")
    if not config.hcg or not config.hcg.neo4j:
        pytest.skip("Integration tests require Neo4j (HCG) configuration")
...
class TestApolloHCGIntegration:
    """Test Apollo HCG integration with real Neo4j database."""
    ...
    def test_get_hcg_entities_from_neo4j(self, integration_client):
        """Test fetching HCG entities directly from Neo4j."""
