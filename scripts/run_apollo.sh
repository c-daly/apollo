...
start_hermes() {
    ...
    if [[ "$status" == "running"* ]]; then
        ...
        # Token for Hermes→Sophia authentication. Fall back to SOPHIA_API_TOKEN
        # (Sophia's gate token) so setting that one documented var is enough for
        # the whole stack; an explicit SOPHIA_API_KEY still takes precedence.
        export SOPHIA_API_KEY="${SOPHIA_API_KEY:-${SOPHIA_API_TOKEN:-sophia_dev}}"
...
start_webapp() {
    ...
    if [[ "$status" == "running"* ]]; then
        ...
        # Generate the webapp's Vite env so the dashboard can reach Sophia
        # (HCG + persona, which are token-gated) and the apollo API (chat +
        # media). .env.local is gitignored and regenerated on each launch from
        # the live ports/token set by start_sophia/start_api above, so a fresh
        # checkout needs no manual setup. Without it the webapp falls back to
        # the wrong host and sends no token -> empty dashboard.
        # Only (re)generate the file we own: write it when absent or empty, or
        # when its first line is our header (so launch-to-launch port/token
        # changes propagate). If a developer has replaced it with their own
        # (a non-empty file without our header), leave it untouched. Values are
        # quoted so a token containing '#' isn't truncated as an inline comment
        # by Vite/dotenv.
        if [ ! -s "${PROJECT_ROOT}/webapp/.env.local" ] || head -n1 "${PROJECT_ROOT}/webapp/.env.local" | grep -q "^# Generated by run_apollo.sh"; then
            cat > "${PROJECT_ROOT}/webapp/.env.local" <<EOF
# Generated by run_apollo.sh -- do not edit; regenerated on each launch.
VITE_SOPHIA_API_URL="http://localhost:${SOPHIA_PORT:-47000}"
VITE_SOPHIA_API_KEY="${SOPHIA_API_TOKEN:-sophia_dev}"
VITE_HCG_API_URL="http://localhost:${APOLLO_PORT:-27000}"
EOF
            log_info "Wrote webapp/.env.local (Sophia :${SOPHIA_PORT:-47000}, API :${APOLLO_PORT:-27000})"
        else
            log_info "Kept custom webapp/.env.local (no run_apollo.sh header; not overwriting)."
        fi
